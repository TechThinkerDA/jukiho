          ...
          <div className="relative">
            ...
            {phases.map((phase, index) => (
              ...
              >
                ...
                {/* Content */}
                <div className={`md:w-1/2 ${index % 2 === 0 ? 'md:pr-12' : 'md:pl-12'} pl-14 md:pl-0 ${index > 0 ? 'md:mt-10' : ''}`}>
                  <div className={`bg-gray-50 dark:bg-gray-900 rounded-lg p-4 shadow-md`}>
                    <div className={`flex items-center mb-2 gap-3 ${index % 2 === 0 ? 'md:justify-between' : ''}`}>
                      {index % 2 === 1 && <div className="md:block hidden">{phase.icon}</div>}
                      <h3 className={`text-2xl font-bold text-gray-800 dark:text-white ${index % 2 === 0 ? 'md:text-right w-full order-2' : ''}`}>
                        {phase.title}
                      </h3>
                      {index % 2 === 0 && <div className="md:block hidden order-3">{phase.icon}</div>}
